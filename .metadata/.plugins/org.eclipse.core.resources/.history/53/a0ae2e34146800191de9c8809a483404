	...
	{	
			 ...
			 Statement statement = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY); )
		{
			...
			try( ResultSet rs = statement.executeQuery(sql))
			{
				...
				if( rs.relative(2))	//Fourth Record
					Program.print(rs);
				
				if( rs.relative(-1))	//Third Record
					Program.print(rs);
			}
		}
		catch( Exception ex )
	...
	{	
		...
		{	
			...
			{				
				...
				if( rs.absolute(2)) //Second Record 
					Program.print(rs);
				if( rs.relative(2)) //Fourth Record 
					Program.print(rs);
				...
				if( rs.absolute(-2)) //Second Last Record 
					Program.print(rs);
				if( rs.relative(-2)) //Moving to 2 previous Record 
					Program.print(rs);
